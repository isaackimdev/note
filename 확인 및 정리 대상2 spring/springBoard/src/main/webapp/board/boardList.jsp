	...
	<tr>
		<td colspan="5" align="center">
			...
			<c:if test="${startPage > 3}">
					[<a id="paging" href="boardList?pg=${startPage-1 }">이전</a>]
			</c:if>
		
			<c:forEach var="i" begin="${startPage }" end="${endPage }" step="1">  
				<c:if test="${pg == i}">
						[<a id="currentPaging" href="boardList?pg=${i }">${i}</a>]
				</c:if>
				<c:if test="${pg != i}">
						[<a id="paging" href="boardList?pg=${i }">${i}</a>]
				</c:if>
				
			</c:forEach>
		
			<c:if test="${endPage < totalP }" >
					[<a id="paging" href="boardList?pg=${endPage+1 }">다음</a>]
			</c:if>
