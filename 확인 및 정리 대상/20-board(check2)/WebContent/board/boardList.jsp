...
<div id="listForm">
<table>
	<tr id="tr_top" style="background-color:orange">
		<td>번호</td>
		<td>제목</td>
		<td>작성자</td>
		<td>날짜</td>
...
<c:forEach var="boardBean" items="${list }">
	<tr align="center">
		<td>${boardBean.board_num }</td> <!-- 변수 이름 직접 입력 >> 자동적으로 getter 함수를 호출한다. -->
		<td align="left">
			...
			▶<a href="boardDetail.do?board_num=${boardBean.board_num }&page=${pageInfo.page}">${boardBean.board_subject }</a> <!-- 글보기에서 원래페이지로 돌아가야할 걍우, 현재페이지 정보 전달 -->
		</td>
